...
library(data4PCCAR)     # Example datasetsfor PCA
...
```{r}
PATH = "C:/_ALEX_/_RESEARCH/_workshops/BrainHack2025/abide-plsc"

# load datasets
covar_df <- read_csv(sprintf("%s/derivatives/gn-reduced/simulated_covariate.csv", PATH))
socio_df <- read_csv(sprintf("%s/derivatives/gn-reduced/simulated_sociocult.csv", PATH))
rsfc_df <- read_csv(sprintf("%s/derivatives/gn-reduced/simulated_rsfc.csv", PATH))
# phy_df <- read_csv(sprintf("%s/derivatives/phyhealth.csv", PATH))

fig_dir <- sprintf("%s/derivatives/gn-reduced/figures/", PATH)
out_dir <- sprintf("%s/derivatives/gn-reduced/", PATH)
